...
inline mat4 rotation_matrix(const quat& rot) {
	...
	const f128 wwww = sse_shuffle<0, 0, 0, 0>(rot.vec_data);
	const f128 xyzw = rot.vec_data;
	const f128 zxyw = sse_shuffle<0, 2, 3, 1>(rot.vec_data);
	const f128 yzxw = sse_shuffle<0, 3, 1, 2>(rot.vec_data);

	const f128 xyzw2 = sse_add(xyzw, xyzw);
	const f128 zxyw2 = sse_shuffle<0, 2, 3, 1>(xyzw2);
	const f128 yzxw2 = sse_shuffle<0, 3, 1, 2>(xyzw2);
